...
- [ ] partial / final 상태 머신
  - 인식된 단어 → `sign:partial` 이벤트 (자막 영역에 즉시 표시)
  - 중복 방지: 직전 인식 단어와 같으면 무시 (같은 수어 반복 전송 방지)
  - 단어 버퍼에 축적 (인식된 단어 리스트 관리)
  - 1.5초간 idle 연속 감지 → 문장화 트리거 (Phase 4의 AI API 호출)
    - AI API 미구현 시 fallback: 단어를 쉼표로 연결하여 `sign:final` 전송
  - final 전송 후 단어 버퍼 초기화, 다음 수어 대기
  - ※ 개별 단어의 final은 없음 — partial은 항상 단어 단위, final은 항상 문장 단위
- [ ] 자막 영역 통합
  - STT 자막과 같은 영역에 표시
